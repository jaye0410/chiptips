<!DOCTYPE html>
<html dir="ltr" lang="ja">
<head>
<!-- Google tag (gtag.js) -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-N7MHQ5W62P"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'G-N7MHQ5W62P');
</script>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, user-scalable=yes, maximum-scale=1.0, minimum-scale=1.0">
<meta name="description" content="SWGoH ChipTips - 銀河チャレンジ | スター・ウォーズ／銀河の英雄(SWGoH)のコンテンツ「銀河チャレンジ」についてのページです。">
<!-- <meta name="keywords" content=""> -->
<title>SWGoH ChipTips - 銀河チャレンジ</title>
<link rel="icon" href="images/favicon.ico" id="favicon">
<link rel="apple-touch-icon" sizes="180x180" href="images/favicon.png">
<link rel="stylesheet" href="css/style.css" type="text/css" media="screen">
<link rel="stylesheet" href="css/custom_table.css" type="text/css" media="screen">
<!--[if lt IE 9]>
<script src="js/html5.js"></script>
<script src="js/css3-mediaqueries.js"></script>
<![endif]-->
<style>
	section.content {
		margin-bottom: 40px;
		background: rgba(255, 255, 255, 0.5);
	}
	section.content p {
		padding: 0 5px;
	}
	span.strong {
		font-weight: bold;
		color: #2b5f2a;
	}
	/* h3.heading {
		margin-bottom: 0 !important;
	} */
	.content h4 {
		font-weight: bold;
		font-size: 16px;
		margin: 5px 0;
	}
	.content h4:before {
		white-space: pre-wrap;
		content: "■ "
	}
	article.gc ul {
		list-style-type: disc;
		padding-right: 5px;
		margin-left: 25px;
		margin-bottom: 20px;
	}
	.gc-nav li, .feat-list li {
		margin-bottom: 5px;
	}
	.feat-list h4 {
		margin-bottom: 0px;
	}
	.feat-list h4:before {
		content: "【"
	}
	.feat-list h4:after {
		content: "】"
	}

	div.youtube {
		margin-bottom: 20px;
	}
	.youtube iframe{
		width: 100%;
		height: 350px;
		border: none;
	}
	/* スマートフォン 横(ランドスケープ) */
	@media only screen and (max-width:640px){
		.youtube iframe{
			width: 100%;
			height: 100%;
			border: none;
		}
	}
	/* スマートフォン 縦(ポートレート) */
	@media only screen and (max-width:480px){
		.youtube iframe{
			width: 100%;
			height: 100%;
			border: none;
		}
	}
</style>
</head>
<body>
<!-- ヘッダー -->
<div id="header">
<?php include "parts/header.html" ?>
</div>
<!-- / ヘッダー -->
<!-- メイン画像 -->
<div id="mainBanner" class="subImg">
	<div class="inner">
		<img src="images/sub_image_gc.webp" width="940" height="300" alt="Sub image">
    <div class="slogan">
			<h2>銀河チャレンジ攻略</h2>
			<h3>
				SWGoH Galactic Challenge</h3>
		</div>
	</div>
</div>
<!-- / メイン画像 -->

<div id="wrapper">
    
<!-- コンテンツ -->
	<section>
		<section class="content">
			<h3 class="heading">メニュー</h3>
			<article class="gc">
				<p>
					このページでは、SWGoHのコンテンツ「銀河チャレンジ」(GC) について説明しています。
				</p>
				<p>
					開催中のGCの攻略記事は
					<a href="./blog/" class="explicitly">&nbsp;ブログ記事&nbsp;</a>
					で投稿しています。基本的には毎回更新しますが、都合により投稿できない場合もあるのでご了承ください。
				</p>

				<hr>

				<h4>目次</h4>
				<ul class="gc-nav">
					<li><a href="#gc_basic" class="explicitly">銀河チャレンジのイベント画像</a></li>
					<li><a href="#gc_level" class="explicitly">銀河チャレンジのレベル</a></li>
					<li><a href="#gc_reward" class="explicitly">銀河チャレンジの報酬</a></li>
					<li><a href="#gc_bonus" class="explicitly">銀河チャレンジのボーナス値</a></li>
					<li><a href="#gc_bonus_list" class="explicitly">銀河チャレンジのボーナス一覧</a></li>
					<li><a href="#gc_feats" class="explicitly">銀河チャレンジの偉業</a></li>
					<li><a href="#gc_feat_list" class="explicitly">銀河チャレンジの偉業一覧</a></li>
				</ul>
			</article>
		</section>

		<section class="content" id="gc_basic">
			<h3 class="heading">銀河チャレンジのイベント画像</h3>
			<article>
				<img src="images/gc_label.webp" width="200" height="300" alt="" class="alignleft frame">
				<p>
					銀河チャレンジの予告期間から開催中にかけて「イベント」メニューを開くと、このようなイベント画像で銀河チャレンジの概要を把握できます。
				</p>
				<h4>惑星名:</h4>
				<p>
					惑星によってボーナス値が変わりますが、イベント内でも改めてボーナス値の確認画面が表示されるため、特に覚えたり意識する必要はありません。
				</p>

				<h4>敵勢力:</h4>
				<p>今回のGCで対峙する敵勢力が分かります。</p>

				<h4>ボーナス付与対象の味方勢力:</h4>
				<p>対象の勢力には強力なボーナス値が付与されますが、
					「フルチーム」の偉業をクリアするためにチームをこの勢力のみで編成する必要があります。
				</p>
				<p>
				前述のとおり、このラベル画像は予告期間中から表示されるので、事前に対象の味方勢力を確認可能です。<br>
				育成不足の場合は開催までの間に装備やモッドなどを整える猶予ができます。
				</p>
			</article>
		</section>

		<section class="content" id="gc_level">
			<h3 class="heading">銀河チャレンジのレベル</h3>
			<article>
				<img src="images/gc_level.webp" width="380" height="200" alt="" class="alignleft frame">
				<p>
					GCのバトルには、I～X (1～10) までのレベルが設定されており、どのレベルからでもバトルを開始できます。<br>
					レベルX (10) をクリアすると、I～IXまでのバトル全てが自動的にクリアされます。<br>
					後述の「偉業」についても同様の扱いになります。
				</p>
				<p>
					レベルが高くなるにつれ敵のレベル、バトルの難易度が高くなりますが、
					その分獲得できる報酬もグレードアップしていきます。
				</p>
				<p>
					GCでは、バトルに負けても開催期間中は無制限に再挑戦できます。<br>
					装備やレリックレベルを期間中に上げるのは難しいかもしれませんが、
					モッドを見直すことで勝利できるケースも多々あります。
					<br>
					最大報酬を目指して、諦めず挑戦してみてください。
				</p>
			</article>
		</section>

		<section class="content" id="gc_reward">
			<h3 class="heading">銀河チャレンジの報酬</h3>
			<article>
				<img src="images/gc_reward.webp" width="320" height="290" alt="" class="alignleft frame">
				<p>
					GCをクリアすると、他のイベントに勝るとも劣らない豪華な報酬が獲得できます。
				</p>
				<p>
					画像の報酬はレベルXクリア時の最大報酬 (赤箱) ですが、
					最大報酬の1ランク下の金箱でもオミクロン資材2つ、カイロテックの回収品が5つ獲得できます。
				</p>
				<p>
					装備ボックスの中にも貴重な装備が複数入っており、さらにモッドスライス資材も潤沢に獲得できます。
					<br>
					非常に時間と労力がかかるアサルトバトルと比べると、コスパの良いイベントではないかと思います。
				</p>

				<h4>モッド報酬:</h4>
				<p>
					有効性モッドが報酬として表示されていますが、獲得できるモッドの種類は惑星によって変わります。
				</p>

				<h4>ユニットのかけら報酬:</h4>
				<p>
					報酬として獲得できるかけらは、敵勢力のユニットが設定されます。
				</p>
			</article>
		</section>

		<section class="content" id="gc_bonus">
			<h3 class="heading">銀河チャレンジのボーナス値</h3>
			<article>
				<img src="images/gc_bonus.webp" width="380" height="200" alt="" class="alignleft frame">
				<p>
					惑星、ボーナス対象の味方勢力、敵勢力によって各ボーナス値が変わります。
				</p>
				<p>
					私のやり方としては、このボーナス値は未確認のまま挑戦する場合が多いです。<br>
					ある程度の種類が繰り返されているので、覚えているものが多いのもひとつの理由ですが、
					一度やってみて勝てない場合は、すべてのボーナス値を確認して攻略のヒントを探します。
				</p>
				<p>
					特に「フルチーム」の偉業クリアに苦労する場合は、「プレイヤー」のボーナス値の確認を強くお勧めします。
				</p>
			</article>
		</section>

		<section class="content" id="gc_bonus_list">
			<h3 class="heading">銀河チャレンジのボーナス一覧</h3>
			<article class="gc">
				<h4>グローバル</h4>
				<ul>
					<li>イラム：「試される力」<br>
						バトル開始時、ユニット全体に10スタック分の<span class="strong">ためらい</span>効果が発生する(戦闘不能になっても持続。耐性発揮、解除、阻止不可)。
						ユニットが特殊スキルを使用するたび、<span class="strong">ためらい</span>のスタックを1解除する。
						スタックが0になると、HPが50%回復し、<span class="strong">クラリティ</span>の効果を得る(戦闘不能になっても持続。コピー、解除、阻止不可)。
						<br>
						<span class="strong">ためらい</span>：
						スタック1つにつき、攻撃力と攻撃力が5%減少する。アシスト、ボーナスターンメーター獲得不可。<br>
						<span class="strong">クラリティ</span>：
						抵抗力が50%、攻撃力が30%上昇する。弱体化効果を耐性発揮するたび、バトル終了までクリティカル率とクリティカルダメージが5%上昇する(スタック可能)。
					</li>
					<li>カミーノ：「基礎訓練」<br>
						リーダースロットのユニットがターンを終了するたび、アクティブな味方ごとに、味方全体が士気のスタックを得る (コピー、解除、阻止不可)。
						ユニットは敵にダメージを与えるたび、または、HPを回復するたび、<span class="strong">士気</span>のスタックを得る。弱体化効果あるいはダメージを受けると、ユニットは士気のスタックを失う。
						ユニットは敵を倒すたび、戦闘不能になった敵の<span class="strong">士気</span>のスタックを全て得る。<br>
						<span class="strong">士気</span>のスタックが10以上あるユニットは、ターンを開始するたび、1ターンのアドバンテージを得る。
						<span class="strong">士気</span>のスタックが15以上あるユニットはターン終了時に、1ターンのプロテクション上昇 (50%) を得る。<br>
						<span class="strong">士気</span>：クリティカルダメージ、防御力、最大HP、有効性、抵抗力がスタックごとに3%上昇する。
					</li>
					<li>キャッシーク：「正当な報復」<br>
						ユニットがダメージを受けるたび、クリティカルヒットを決めるまでユニットのクリティカルダメージが20%上昇する (スタック可能)。<br>
						ユニットが弱体効果を受けるたび、ユニットのプロテクションが10%回復する。<br>
						ユニットが戦闘不能になるたび、ランダムな他の味方1体にボーナスターンが発生する。
					</li>
					<li>ダソミア：「闇魔術」<br>
						10ターンごとに、戦闘不能になったユニット全体がHP50%の状態で復活する。
					</li>
					<li>ホス：
						<ul>
							<li>「小惑星帯」<br>
							1ターン終了するたび、戦闘不能になるまでユニット全体に時間経過ダメージが発生する（耐性発揮不可）。</li>
							<li>「高性能照準コンピューター」<br>
							サポートの各ターン開始時に、ランダムな敵1機に2ターンの間、ターゲットロックを発生させる。
							ターゲットロック状態の敵が攻撃を受けるたび、50%の確率で味方をアシストに呼ぶ。</li>
						</ul>
					</li>
				</ul>

				<h4>プレイヤー</h4>
				<ul>
					<li>シス：「残酷な陰謀」<br>
						味方シスが戦闘不能になるたび、味方シス全体の最大HPと攻撃力がバトル終了まで20%上昇(スタック可能)し、
						HPが25%回復する。敵を倒すたび、味方シスは<span class="strong">ダーク・アセンション</span>を得る(スタック可能。コピー、解除、阻止不可)。
						<br>
						<span class="strong">ダーク・アセンション</span>：受けるダメージが15%減少し、与えるダメージが30%上昇する。
					</li>
					<li>帝国軍トルーパー：「帝国軍アーマー」<br>
						帝国軍トルーパーの最大HPと最大プロテクションが15%上昇する。
						リーダースロットにいないドロイド以外の帝国軍トルーパーは、初めて戦闘不能になると、HP100%、プロテクション75%の状態で復活する。
						帝国軍トルーパーが復活するたび、1ターンの間、敵全体によろめきと無防備を発生させる(耐性発揮不可)。
						ターンメーターが100%を下回る帝国軍トルーパーは、ボーナスターンメーターを得るたび、HPとプロテクションが5%回復し、1ターンの間、クリティカルダメージが10%上昇する(スタック可能)。
						帝国軍トルーパーはターン外に攻撃するたび、1ターンの間、2%のボーナスプロテクションを得る(スタック可能)。
					</li>
					<li>反乱軍ファイター：「ピンポイント」<br>
						反乱軍ファイターが特殊スキルを使用するたび、<span class="strong">ピンポイント</span>状態の敵がいない場合、
						ターゲットに2ターンの<span class="strong">ピンポイント</span>を発生させる (解除、阻止、耐性発揮不可)。
						<span class="strong">ピンポイント</span>状態の敵は、反乱軍ファイターからダメージを受けるたび、HPが2%減少する。
						反乱軍ファイターは<span class="strong">ピンポイント</span>状態の敵に通常スキルを使用するたび、時間経過回復のスタックを獲得し、
						<span class="strong">ピンポイント</span>状態の敵に特殊スキルを使用するたび、時間経過回復が発生している反乱軍ファイターの味方全体をアシストに呼ぶ。
						<br>
						反乱軍ファイターは、<span class="strong">ピンポイント</span>状態の敵を倒すたびボーナスターンを得て、
						反乱軍ファイター全体は2ターンの間アドバンテージ、クリティカルダメージ上昇、スピード上昇を得る。
						時間経過回復のスタックが10以上ある反乱軍ファイターは、HPとプロテクションが50%の状態で復活する。
						ユニットの<span class="strong">ピンポイント</span>の効果が切れるたび、そのユニットのHPが100%回復する。<br>
						<span class="strong">ピンポイント</span>：敵の攻撃にボーナス効果が発生する。
					</li>
					<li>ファースト・オーダー：
						<ul>
							<li>「ブラインドフューリー」<br>
							アドバンテージを得るたび、味方ファースト・オーダーのクリティカル率が20%減少し、クリティカルダメージが35%上昇する。
							ファースト・オーダーがターン中に1度もクリティカルヒットを与えず、またアドバンテージが未発生の場合はターン終了時に
							2ターンのアドバンテージを得る。</li>
							<li>「容赦なき追撃」<br>
							味方ファースト・オーダーがクリティカルヒットを決めるたび、その味方は2ターンの間、プロテクション上昇（25%）を得る。
							ハント状態の敵が味方ファースト・オーダーに与えるダメージが50%減少する。
							味方ファースト・オーダーがハント状態の敵を倒した場合、その味方のHPとプロテクションが100%回復する。</li>
						</ul>
					</li>
				</ul>

				<h4>敵</h4>
				<ul>
					<li>銀河共和国：「揺るぎなき報復」<br>
						味方銀河共和国は敵からダメージを受けるたび、受けたダメージの100%分その敵に固定ダメージを与える。
						このスキルで与えるダメージは、味方がこの方法でダメージを与えるごとに10%上昇する。
						このダメージは回避できず、また敵を戦闘不能にできない。
					</li>
					<li>クローン・トルーパー：「勇猛」<br>
						バトル終了まで、クローン・トルーパーは強化効果(解除可能)を受けるたび、「<span class="strong">勇猛</span>」のスタックを1得る(コピー、解除不可)。
						<br>
						<span class="strong">勇猛</span>：攻撃力、クリティカル率、クリティカルダメージが5%、ヘルススティールが1%上昇する。
					</li>
					<li>ドロイド：「プロトコル: 根絶」<br>
						味方ドロイドの攻撃力が30%上昇する。
						味方ドロイドがターン中にクリティカルヒットを決めるたび、その敵にターゲットロックが発生していない場合は2ターンのターゲットロックを発生させる。
						味方ドロイドがターゲットロック状態の敵にダメージを与えるたび、その味方のプロテクションが10%回復する。
					</li>
					<li>反乱軍：「オッズ（シップ）」<br>
						味方反乱軍はターン中にクリティカルヒットを決めるたび、100%の確率で再び攻撃する。
						このボーナス攻撃の確率は次回以降は10%に減少する。味方反乱軍はカウンターされない。
					</li>
					<li>レジスタンス：「勇猛不屈の精神」<br>
						敵が味方レジスタンスからのマイナス効果に耐性発揮するたび、その味方に1ターンの時間経過回復が発生する。
						味方レジスタンスが強化効果を得るたび、味方の有効性、クリティカル率、クリティカルダメージがバトル終了まで
						5%上昇(スタック可能)し、ターンメーターが10%上昇する。
					</li>
				</ul>
			</article>
		</section>

		<section class="content" id="gc_feats">
			<h3 class="heading">銀河チャレンジの偉業</h3>
			<article>
				<img src="images/gc_feats.webp" width="380" height="200" alt="" class="alignleft frame">
				<p>
					各レベルのクリアと「フルチーム」の偉業は毎回固定です。<br>
					他2種類の偉業は開催ごとに変わります。
				</p>
				<p>
					最も重要な偉業は「フルチーム」で、赤箱に到達するためにはレベルXのクリアが必須です。<br>
					フルチームのレベルXをクリアすれば、残りの偉業の内1つはレベルIXでも赤箱に到達できます。
				</p>
				<p>
					どうしてもフルチームがクリアできない場合は金箱を目指すことになりますが、
					その際フルチームはレベルVI、その他偉業をレベルXのクリアで到達できます。
				</p>
			</article>
		</section>

		<section class="content" id="gc_feat_list">
			<h3 class="heading">銀河チャレンジの偉業一覧</h3>
			<article class="gc">
				<p>※五十音順</p>
				<ul class="feat-list">
					<li><h4>完全勝利</h4>
						ユニットを失わずにバトルをクリアする</li>
					<li><h4>危険予測</h4>
						フルチームではない編成でバトルをクリアする</li>
					<li><h4>クリティカルストリーク</h4>
						味方のターン中にクリティカルヒットを3連続決めてバトルをクリアする</li>
					<li><h4>サーマル・デトネーターを持ってるぞ！：</h4>
						サーマル・デトネーターを40回発生させてバトルをクリアする</li>
					<li><h4>しびれる戦術</h4>
						30回スタンさせてバトルをクリアする</li>
					<li><h4>自由</h4>
						リーダースロットにリーダーを使用せずに勝利する</li>
					<li><h4>シンプルトリック</h4>
						特殊スキルの使用3回以内でバトルに勝利する</li>
					<li><h4>勢力均衡</h4>
						ライトサイドユニット1体以上とダークサイドユニット1体以上を生存させて勝利する</li>
					<li><h4>チェインアタック</h4>
						8ターン連続で取ってから勝利する</li>
					<li><h4>挑発測定</h4>
						挑発を5回得てバトルをクリアする</li>
					<li><h4>長引く死</h4>
						時間経過ダメージを30回発生させてバトルをクリアする</li>
					<li><h4>バランス戦略</h4>
						チームにライトサイド、ダークサイドユニットを1体ずつ入れ、いずれも生存した状態でバトルに勝利する</li>
					<li><h4>妨害</h4>
						ターンメーターを400%減少させてバトルに勝利する</li>
					<li><h4>無防備</h4>
						チームにタンクを使用せずにバトルに勝利する</li>
					<li><h4>闇討ち</h4>
						ステルスを10回得てバトルをクリアする</li>
					<li><h4>ラッキーショット</h4>
						クリティカルヒットを30回決めてバトルをクリアする</li>
				</ul>
			</article>
		</section>
    
	</section>
	<!-- / コンテンツ -->
</div>
<!-- / WRAPPER -->

<!-- フッター -->
<div id="footer">
	<?php include "parts/footer.html" ?>
</div>
<!-- / フッター -->

<script src="js/common.js"></script>
<script>
	const page = window.location.pathname.split("/").pop();
	changeActivePage(page);
</script>
</body>
</html>